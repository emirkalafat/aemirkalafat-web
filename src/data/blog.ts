...
- Correlation with known AES operations`,
  },
  {
    ...
    description:
      "Stripping binary bloat and leveraging link-time optimization (LTO) to squeeze massive Rust binaries onto microcontrollers with less than 64KB of flash memory.",
    relatedIds: ["LOG-004", "LOG-005"],
    markdown: `# Optimizing Rust Compiles for Embedded Systems

Rust's zero-cost abstractions come with a hidden cost: compile-time code generation that can balloon binary sizes. On a microcontroller with 64KB of flash, every byte counts. The default release profile with opt-level = "z" and LTO enabled is a good start, but it is rarely sufficient.
...
Setting codegen-units = 1 forces LLVM to optimize the entire binary as a single compilation unit. Combined with fat LTO, this enables cross-crate inlining and dead code elimination that can reduce binary size by 30–60% compared to the debug profile.`,
  },
  {
    id: "LOG-002",
    category: "THEORY",
    ...
    description:
      "An algorithmic approach to mapping complex topologies without relying on standard Cartesian grids. Implications for rendering engines and collision detection in VR environments.",
    relatedIds: ["LOG-005", "LOG-003"],
    markdown: `# Non-Euclidean Data Structures in Spatial Computing

Standard R-trees and k-d trees assume Euclidean distance as the primary partitioning metric. When dealing with hyperbolic or spherical spaces — as encountered in physics simulations, certain graph embeddings, and non-Euclidean VR environments — these structures degrade to O(n) worst-case queries.

## Gyrovector Spaces as a Framework

Gyrovector spaces, formalized by Abraham Ungar, provide an algebraic framework for hyperbolic geometry analogous to how vector spaces formalize Euclidean geometry. The Möbius addition operation replaces standard vector addition, enabling the construction of spatial indices that preserve the geometry of the underlying space.

## Practical Collision Detection

For collision detection in a hyperbolic space rendered on a Poincaré disk, the naive approach of projecting to Euclidean coordinates and back introduces cumulative floating-point errors. Instead, maintaining objects in native gyrovector coordinates and computing geodesic distances directly reduces error accumulation significantly.`,
  },
  {
    id: "LOG-003",
    category: "E-ENG",
    ...
    description:
      "Designing physical structures that manipulate sound waves below sub-wavelength frequencies. Applications in EMI shielding for high-density PCB layouts.",
    relatedIds: ["LOG-FEAT-001", "LOG-005"],
    markdown: `# Acoustic Metamaterials & Signal Dampening
...
The same phononic bandgap principle that blocks acoustic waves in a specific frequency range can be applied to electromagnetic interference. By designing PCB substrate geometries with periodic inclusions at the millimeter scale, it is possible to create stopbands that attenuate RF noise from high-speed switching regulators before it couples into sensitive analog circuits.`,
  },
  {
    ...
    description:
      "Bypassing userspace overhead with io_uring and AF_XDP to build high-frequency data pipelines that route packets at near-wire speed without CPU bottlenecks.",
    relatedIds: ["LOG-001", "LOG-WIDE-001"],
    markdown: `# Zero-Copy Networking in Linux Kernel Space

Traditional networking in Linux copies packet data between kernel and userspace at least twice per packet. At 10Gbps line rates with 64-byte packets, this overhead becomes the bottleneck. AF_XDP bypasses the kernel networking stack entirely by mapping packet buffers directly to userspace memory via a shared UMEM region.

## UMEM and Ring Buffer Setup
...
\`\`\``,
  },
  {
    ...
    description:
      "Why rely on FreeRTOS when you can build your own preemptive scheduler? A step-by-step guide to context switching, managing stack frames, and implementing basic mutexes on an ATmega328P.",
    isWide: true,
    relatedIds: ["LOG-001", "LOG-004"],
    markdown: `# Building a Custom RTOS from Scratch

A preemptive scheduler on a bare-metal AVR microcontroller requires three primitives: a tick interrupt to drive task switching, a context save/restore mechanism that preserves all CPU registers, and a ready queue to determine which task runs next. Everything else — mutexes, semaphores, message queues — is built on top of these three.

## Context Switching on AVR
...
The key insight is that each task maintains its own stack. The scheduler's context switch simply saves the current SP into the current task's TCB, loads the next task's SP, and restores its register context. From the task's perspective, it resumes exactly where it was suspended.`,
  },
];
